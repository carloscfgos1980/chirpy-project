...
6. <main.go> Create the router:
 mux.HandleFunc("POST /api/chirps", apiCfg.handlerChirpsCreate)

# 5.7 Get all chirps

1. <sql/queries.sql> Create a querie to get all chirps
-- name: GetChirps :many
SELECT * FROM chirps
ORDER BY created_at ASC;

2. Generate code to handle the method in order to get all chirps in database with the querie. In "server" a root directory:
sqlc generate

3. <handler_chirps_get.go>. Create the method to get aal chirps:
func (cfg *apiConfig) handlerChirpsRetrieve(w http.ResponseWriter, r*http.Request) {

    //get the collection of chirps
...
// initiane a variable that is an slice of Chirp struct. I created this struct in the handler to create chirps in the global scope so I could use in other part of the api
 chirps := []Chirp{}

 // loop over the collection of chirps to populate the variable that created
...
4. <main.go> Create the router:
 mux.HandleFunc("GET /api/chirps", apiCfg.handlerChirpsRetrieve)

# 5.8 Get a chirp

1. <sql/queries.sql> Create the querie to get a single chirp
-- name: GetChirp :one
SELECT * FROM chirps
WHERE id = $1;

2. Generate code to handle the method in order to a chirp in database with the querie. In "server" a root directory:
sqlc generate

3. <handler_chirp_get.go> Creata a method to get a chirp:
...
// get the valye from the database
...
// response with ok and populate the Chirp struct with the value from the database
